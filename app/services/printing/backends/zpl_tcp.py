...
class ZplSocketBackend:
    ...
    def _probe(self) -> tuple[bool, str]:
        ...
        try:
            with socket.create_connection(
                (self._opts.host, self._opts.port), timeout=self._opts.timeout_seconds
            ):
                return True, ""
    ...
    def print_label(self, request: LabelPrintRequest) -> PrintResult:
        ...
        # The check a socket backend genuinely can make: the raster we are about
        # to send must fit the label we were told is loaded. Catches an operator
        # who configured a bigger label than the guards saw.
        ...
        try:
            with socket.create_connection(
                ...
                sock.settimeout(self._opts.timeout_seconds)
